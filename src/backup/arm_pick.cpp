...
void executeTargetCB(const armadillo2_bgu::SimpleTargetGoalConstPtr& goal, target_server_t* as)
{
    if (executing_pick || executing_place)
    {
        armadillo2_bgu::SimpleTargetResult result;
...
void executePlaceCB(const armadillo2_bgu::SimplePlaceGoalConstPtr& goal, place_server_t* as)
{
    if (executing_pick || executing_target)
    {
        armadillo2_bgu::SimplePlaceResult result;
        as->setAborted(result, "tried to execute target, while some other goal");
        return;
    }
...
int main(int argc, char** argv)
{
    ...
    while (ros::ok())
    {
        ...
        if (objects_map.size() > 0)
        {
            ...
            if (executing_place)
            {
                armadillo2_bgu::SimplePlaceFeedback feedback;
            ...
            {
                armadillo2_bgu::SimplePickFeedback feedback;
                feedback.distance = distance;
                feedback.x = eef_pose.pose.position.x;
                feedback.y = eef_pose.pose.position.y;
                feedback.z = eef_pose.pose.position.z;
            ...
            {
                armadillo2_bgu::SimpleTargetFeedback feedback;
                feedback.distance = distance;
                feedback.x = eef_pose.pose.position.x;
                feedback.y = eef_pose.pose.position.y;
                feedback.z = eef_pose.pose.position.z;
