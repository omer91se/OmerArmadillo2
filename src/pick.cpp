...
void place_cb(const armadillo2_bgu::OperationGoalConstPtr& goal, Server* as){
    ...
    if(!gotPlace) {
        ...
        std::cout << "goal->x:" << x << "\ngoal->y" << y << "\ngoal->z:" << z << std::endl;
        origin_goal.point.x = place_x;
        origin_goal.point.y = place_y;
        origin_goal.point.z = place_z;
        ROS_INFO("[arm_server]: goal is set ");
        ...
        simple_place_goal.frame_id = transformed_goal.header.frame_id;
        simple_place_goal.obj_name = "target";
        simple_place_goal.x = transformed_goal.point.x;
        simple_place_goal.y = transformed_goal.point.y;
        simple_place_goal.z = transformed_goal.point.z;

        ROS_INFO("[pick]: Trying to Place at: [x,y,z,]=[%f,%f,%f]" ,x,y,z );
        // goal.= res->res;
